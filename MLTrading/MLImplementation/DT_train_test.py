...
#class Decision_Tree_ID3(object):
    ...
    def parseFile_mix(filename,begin,num):
        # //attr_Info=attr_pattern()
        #//train_file=open(filename,'rU')
        #//Train_Data=np.loadtxt(train_file, dtype=str, delimiter=',')
        #//num,attribute=Train_Data.shape
        ...
        for i in range(begin,begin+num):
                y.append(matrix[i].split(',')[0].strip())
                X.append(matrix[i].split(',')[1:])
...
if __name__ == '__main__':
    ...
    data_num=filenum('mush_all.data')
    ratio=0.1
    train_num= int(data_num*ratio)
    test_num=data_num-train_num
    test_begin=train_num
    attr_L,X,y=parseFile_mix('mush_all.data',0, train_num)
    tree=create_Tree(X,y,attr_L)
    attr_test,X_test,y_test = parseFile_mix('mush_all.data',test_begin,test_num)
