...
'''
#Note: Have to change the attribute by hand, when want to attain one-node tree.
...
#class Decision_Tree_ID3(object):
    ...
    def create_One_Node_Tree(X,y,attr):
        if len(set(y))==1:
            return y[0]
        if ratio(y)>0.9:
            return get_Majority
        ...
        #for i in attr_L:
        split_dict,sub_Label=split_into_subSet(X,y,attr)
        ...
        for attr_val, (X_sub,y_sub) in split_dict.items():
            #recursive
            tree[attr][attr_val] = create_One_Node_Tree(X_sub,y_sub,attr)
    ...
    def create_Tree(X, y, attr_L):
        ...
        for attr_val, (X_sub,y_sub) in split_dict.items():
            #recursive
            tree[attr][attr_val] = create_Tree(X_sub,y_sub,attr)
        print tree
        return tree #return tree{,{}}

    def traverse(x,tree,i):
        #select featrues
        if isinstance(tree,str):
            y_label[i].append(tree)
        else:
            ...
            traverse(x,subtree,i)

    def predict_Tree(X_test,tree,i):
        for x in range(len(X_test)):
            traverse(X_test[x],tree,i)

    def test_Tree(y_test,k):
        error=0.0
        for i in range(len(y_test)):
            #print len(y_label[k]),len(y_test)
            if(y_test[i]!=y_label[k][i]):
                error+=1
...
if __name__ == '__main__':
    ...
    for i in attr_L:
        ...
        attr = attr_L[node_attr_idx]
        print i
        All_tree[attr]=create_One_Node_Tree(X,y,attr)
        print All_tree[attr]
        predict_Tree(X_test,All_tree[attr],i)

        test_accuracy[i]=test_Tree(y_test,i)
        print test_accuracy[i]
        if i==4:
            with open('mush.dot','w') as f:
                dot=dotify(All_tree[i])
                f.write(dot)
        print "i:",i
